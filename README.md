...
|   - loads feature files from assets on Android/iOS                         |
|   - loads feature files from disk on desktop-style platforms               |
|                                                                            |
...
1. The runner reads a `.feature` file.
   - Android/iOS: from Flutter assets using `rootBundle.loadString()`
   - Desktop-style platforms: from disk using `File.readAsString()`
...
### Register feature files as assets on Android/iOS

On Android and iOS, `.feature` files are bundled inside the application and
must be declared as Flutter assets:
...
Without this, `runFeatureFile()` cannot load the feature on mobile.
...
`runFeatureFile()` loads feature files differently depending on platform:

- Android/iOS: from the Flutter asset bundle
- Desktop-style platforms: from the local file system
...
### `PathNotFoundException` or "Could not load feature file"

- On Android/iOS, make sure your `.feature` files are declared under
  `flutter: assets:` in the consumer app `pubspec.yaml`
- On desktop-style platforms, make sure the path passed to `runFeatureFile()`
  exists on disk
